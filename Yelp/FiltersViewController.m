...
@property (strong, nonatomic) YelpCategories const* categories;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        // TODO: this is probably pretty slow.  figure out what the right paradigm is for doing this
        self.categories = [[YelpCategories alloc] init];
        self.selectedSortByIndex = 0;
        self.selectedDistanceIndex = 0;
        
        // TODO: separate the management of this stuff to separate classes
        ...
        self.toggleOptions = @[@[@"deals_filter",@"Offering a Deal"], @[@"kicks",@"Second Switch just for Kicks"]];
...
- (void)viewDidLoad
{
    ...
    UINib *expandableCellNib = [UINib nibWithNibName:@"ExpandableCell" bundle:nil];
    [self.table registerNib:expandableCellNib forCellReuseIdentifier:@"ExpandableCell"];
    
    UINib *toggleCellNib = [UINib nibWithNibName:@"ToggleCell" bundle:nil];
    [self.table registerNib:toggleCellNib forCellReuseIdentifier:@"ToggleCell"];
...
- (void) setDelegate:(id<FiltersDelegate>)delegate
{
    ...
    if (delegate.filters[@"category_filter"]) {
        self.categories = [[YelpCategories alloc] initWithApiParam:delegate.filters[@"category_filter"]];
    }
...
{
    NSLog(@"Toggling row %d", sender.tag);
    ...
    switch (indexPath.section) {
        case SortBySection: {
            if (self.sortByExpanded) {
                ...
                ExpandableCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ExpandableCell"];
        ...
        case MiscSection: {
            ToggleCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ToggleCell"];
